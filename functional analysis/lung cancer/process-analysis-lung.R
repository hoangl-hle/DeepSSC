...
#[1] "LASS3"     "FLJ90757"  "LOC284578" "CALML3"    "KRTAP19-1" "LOC152217"

top10 = unique(top10)
length(top10) #14 genes
...
#                             Heatmap
#-------------------------------o0o-------------------------------#
exp = read.table("GE_LUNG_CANCER.csv", check.names = F, header = T, row.names = 1, sep=",") %>% t()
label=read.table("LABEL.csv", check.names = F, header = T, row.names =1, fill = T,
                 sep=",")

exp = exp[rownames(exp) %in% top10,]

colnames(label)[1] = 'label'
label$label = ifelse(label$label == "lung adenocarcinoma", "ADC", "SCC")
...
ha = HeatmapAnnotation(subtypes = info2$groups,
                       col = list(groups = c("ADC" = "turquoise", "SCC " = "black")))
...
h_list = ha %v% hm
draw(h_list, column_title = "1010 patients",column_title_side = "bottom",
